...
const ObservatoryAnimation = (function () {
    ...
    // ══════════════════════════════════════════════════════════════
    //  OBSERVATORY SCENE — moonlit dome with realistic telescope
    // ══════════════════════════════════════════════════════════════
    function drawObservatory(t, elapsed, zoom) {
        ...
        if (candleP > 0) {
            for (let side = 0; side < 2; side++) {
                ...
                for (let i = 0; i < candles.length; i++) {
                    const c = candles[i];
                    const cx_ = bx + (i / candles.length) * w * 0.12;
                    const bTop = cy - (Math.max(c.open, c.close) - 0.35) * cS;
                    const bBot = cy - (Math.min(c.open, c.close) - 0.35) * cS;
                    const wTop = cy - (c.high - 0.35) * cS;
                    const wBot = cy - (c.low - 0.35) * cS;
                    const cA = candleP * 0.45;
                    const col = c.bull ? `rgba(50, 210, 100, ${cA})` : `rgba(210, 50, 80, ${cA})`;
                    ctx.beginPath();
                    ctx.moveTo(cx_, wTop);
                    ctx.lineTo(cx_, wBot);
                    ctx.strokeStyle = col;
                    ...
                    ctx.stroke();
                    ctx.fillStyle = col;
                    ctx.fillRect(cx_ - 2, bTop, 4, Math.max(1, bBot - bTop));
                }
            }
        }
    ...
    // ══════════════════════════════════════════════════════════════
    //  PULL-BACK — cosmos framed in telescope aperture disc
    // ══════════════════════════════════════════════════════════════
    function drawPullback(t, elapsed) {
        ...
        const cx = w / 2, cy = h / 2;
        const minDim = Math.min(w, h);
        const discR = minDim * lerp(0.54, 0.3, easeInOut(t));

        // Circular mask — dark outside the disc
        ctx.beginPath();
        ctx.rect(0, 0, w, h);
        ctx.arc(cx, cy, discR, 0, Math.PI * 2, true);
        ctx.fillStyle = `rgba(6, 8, 14, ${a * 0.97})`;
        ctx.fill();

        // Soft vignette at edge
        const vg = ctx.createRadialGradient(cx, cy, discR * 0.8, cx, cy, discR);
        vg.addColorStop(0, 'rgba(0,0,0,0)');
        vg.addColorStop(1, `rgba(6, 8, 14, ${a * 0.5})`);
        ctx.fillStyle = vg;
        ctx.beginPath();
        ctx.arc(cx, cy, discR, 0, Math.PI * 2);
        ctx.fill();

        // Polished metal lens ring
        ctx.beginPath();
        ctx.arc(cx, cy, discR, 0, Math.PI * 2);
        const ringG = ctx.createRadialGradient(cx, cy - discR * 0.3, discR * 0.8, cx, cy, discR + 5);
        ringG.addColorStop(0, 'rgba(110, 105, 92, 0)');
        ringG.addColorStop(0.9, `rgba(100, 95, 82, ${a * 0.5})`);
        ringG.addColorStop(1, `rgba(65, 60, 52, ${a * 0.3})`);
        ctx.strokeStyle = `rgba(115, 108, 92, ${a * 0.5})`;
        ctx.lineWidth = 3.5;
        ctx.stroke();

        // Outer barrel
        ctx.beginPath();
        ctx.arc(cx, cy, discR + 7, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(55, 52, 45, ${a * 0.35})`;
        ctx.lineWidth = 10;
        ctx.stroke();

        // Dome reflection on lens
        if (t > 0.3) {
            const refA = smoothstep(0.3, 0.65, t) * 0.1;
            ctx.beginPath();
            ctx.ellipse(cx, cy - discR * 0.28, discR * 0.55, discR * 0.12, 0, 0, Math.PI);
            ctx.strokeStyle = `rgba(120, 115, 100, ${refA})`;
            ctx.lineWidth = 1.5;
            ctx.stroke();
            ctx.beginPath();
            ctx.ellipse(cx, cy - discR * 0.12, discR * 0.3, discR * 0.18, 0, Math.PI, 0);
            ctx.strokeStyle = `rgba(85, 80, 68, ${refA * 0.5})`;
            ctx.lineWidth = 1;
            ctx.stroke();
        }
    ...
    function render(timestamp) {
        ...
        const phases = [
            [drawObservatory, 0, 8000],           // 0-8s: Observatory scene
            [drawApproach, 6000, 14000],           // 6-14s: Zoom into eyepiece
            [drawOptics, 12500, 21000],            // 12.5-21s: Inside optics
            [drawDataTransform, 19500, 27500],     // 19.5-27.5s: Data transformation
            [drawCosmos, 26000, 39500],            // 26-39.5s: Financial cosmos
            [drawPullback, 37000, 42000]           // 37-42s: Pull-back reveal
        ];
